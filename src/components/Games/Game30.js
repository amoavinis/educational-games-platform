...
import { game30Items, game30Answers } from "../Data/Game30Data";
import useAudio from "../../hooks/useAudio";
import bravoAudio from "../../assets/sounds/general/bravo.mp3";
import practiceEnd from "../../assets/sounds/general/end-of-practice.mp3";

const Game30 = ({ gameId, schoolId, studentId, classId }) => {
  ...
  const items = useMemo(() => game30Items, []);
    ...
    if (!currentItem.isExample) {
      ...
      setGameStats((prev) => ({
        ...
        rounds: [
          ...prev.rounds,
          {
            question: `${currentItem.first} – ${currentItem.second}`,
    ...
    <Container fluid className="game-container">
      ...
      <Row className="game-row-centered">
        <Col md={12} lg={10}>
          ...
          <Card className="main-card">
            <Card.Header className="text-center" style={{ backgroundColor: "#2F4F4F", color: "white" }}>
              <h4 className="mb-0 game-title-header">Επιλέγω αν ταιριάζουν τα ζευγάρια λέξεων</h4>
            </Card.Header>
            <Card.Body>
              <div className="p-4 bg-light rounded mb-4 text-center">
                <div className="display-4 font-weight-bold">
                  {currentItem.first} – {currentItem.second}
                </div>
              </div>

              <Row className="g-3 mb-4 answer-options">
                {game30Answers.map((option, index) => {
