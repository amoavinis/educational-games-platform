...
import { game31Items, game31Answers } from "../Data/Game31Data";
...
const Game31 = ({ gameId, schoolId, studentId, classId }) => {
  ...
  const items = useMemo(() => game31Items, []);
    ...
    <Container fluid className="game-container">
      ...
      <Row className="game-row-centered">
        <Col md={12} lg={10}>
          ...
          <Card className="main-card">
            <Card.Header className="text-center" style={{ backgroundColor: "#2F4F4F", color: "white" }}>
              <h4 className="mb-0 game-title-header">Απαντάω όσο πιο γρήγορα μπορώ αν η πρόταση βγάζει νόημα</h4>
            </Card.Header>
            <Card.Body>
              <div className="p-4 bg-light rounded mb-4 text-center">
                <div className="display-6 font-weight-bold">{currentItem.sentence}</div>
              </div>

              <Row className="g-3 mb-4 answer-options">
                {game31Answers.map((option, index) => {
