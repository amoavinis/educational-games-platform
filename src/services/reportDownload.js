...
export const downloadReportsWithAudio = async (reportData, schools, games, selectedSchool, selectedGame) => {
  try {
    ...
    // Headers
    const headers = ["Μαθητής", "Τάξη", "Ημερομηνία/Ώρα", "Φύλο", "Ημερομηνία Γέννησης", "Διάγνωση"];
    ...
    reportData.forEach((report) => {
      ...
      for (let i = 0; i < maxQuestions; i++) {
        ...
        if (question) {
          row.push(
            ...
            question.correct !== undefined ? (question.correct ? "Ναι" : "Όχι") : "N/A",
      ...
      if (report.parsedResults?.audioDownloadURL) {
        const audioPromise = downloadAudioFile(
          ...
          `${report.studentName}_${report.className}_${report.parsedResults.datetime?.replace(/[:\s]/g, '-')}.webm`
        ).then(({ blob, fileName }) => {
          audioFolder.file(fileName, blob);
          console.log(`Added audio file: ${fileName}`);
        }).catch((error) => {
          console.error(`Failed to download audio for ${report.studentName}:`, error);
        });
    ...
    const excelFilename = `Αναφορα_${selectedGameName.replace(/\s+/g, "_")}_${dateStr}.xlsx`;
    ...
    const zipFilename = `Reports_with_Audio_${selectedGameName.replace(/\s+/g, "_")}_${dateStr}.zip`;
